...
void SlidingGateComponent::setup() {
  ...
  this->pin_dir_0_->attach_interrupt<SlidingGateComponent>(
      &SlidingGateComponent::handle_interrupt, this, gpio::INTERRUPT_ANY_EDGE);
  this->pin_dir_1_->attach_interrupt<SlidingGateComponent>(
      &SlidingGateComponent::handle_interrupt, this, gpio::INTERRUPT_ANY_EDGE);
...
void SlidingGateComponent::publish(bool force) {
  if (force || (this->reported_position != this->position) ||
...
void SlidingGateComponent::control(const cover::CoverCall& call) {
  ...
  if (call.get_stop()) {
    ...
    ESP_LOGD(
        TAG, "control: control_target_position: %.0f%%", this->control_target_position * 100.0f);
